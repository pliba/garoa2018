...
def test_evaluate_not_operator():
    ...
    with raises(errors.InvalidOperator):
        evaluate(expr)
...
def test_evaluate_not_operator_message():
    ...
    with raises(errors.InvalidOperator) as excinfo:
        evaluate(expr)
...
def test_evaluate_unknown_operator():
    expr = parse(tokenize("@"))
    with raises(errors.UnknownOperator) as excinfo:
        evaluate(expr)
...
def test_evaluate_empty_expression():
    expr = parse(tokenize("()"))
    with raises(errors.EmptyExpression):
        evaluate(expr)
